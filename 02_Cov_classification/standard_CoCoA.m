function [dualGap,tOP,T] = standard_CoCoA(Dataset, MaxItrOut, weight_Cen_Sub,H)
%% Standard CoCoA
...
for T=1:MaxItrOut
    ...
    %% local worker #1
    kset11=[1:round(m/K)];
    [alphaTri11,wTri11,tAvg11]=local_SVM(w,alpha11,m,lambda,A(:,kset11),y0(kset11,1),H);
    alpha11 = alpha11 + alphaTri11;
    %% local worker #2
    kset12=[round(m/K)+1:2*round(m/K)];
    [alphaTri12,wTri12,tAvg12]=local_SVM(w,alpha12,m,lambda,A(:,kset12),y0(kset12,1),H);
    alpha12 = alpha12 + alphaTri12;
    %% local worker #3
    kset13=[2*round(m/K)+1:3*round(m/K)];
    [alphaTri13,wTri13,tAvg13]=local_SVM(w,alpha13,m,lambda,A(:,kset13),y0(kset13,1),H);
    alpha13 = alpha13 + alphaTri13;
    %% local worker #4
    kset14=[3*round(m/K)+1:4*round(m/K)];
    [alphaTri14,wTri14,tAvg14]=local_SVM(w,alpha14,m,lambda,A(:,kset14),y0(kset14,1),H);
    alpha14 = alpha14 + alphaTri14;
    
    %% local worker #5
    kset21=[4*round(m/K)+1:5*round(m/K)];
    [alphaTri21,wTri21,tAvg21]=local_SVM(w,alpha21,m,lambda,A(:,kset21),y0(kset21,1),H);
    alpha21 = alpha21 + alphaTri21;
    %% local worker #6
    kset22=[5*round(m/K)+1:6*round(m/K)];
    [alphaTri22,wTri22,tAvg22]=local_SVM(w,alpha22,m,lambda,A(:,kset22),y0(kset22,1),H);
    alpha22 = alpha22 + alphaTri22;
    %% local worker #7
    kset23=[6*round(m/K)+1:7*round(m/K)];
    [alphaTri23,wTri23,tAvg23]=local_SVM(w,alpha23,m,lambda,A(:,kset23),y0(kset23,1),H);
    alpha23 = alpha23 + alphaTri23;
    %% local worker #8
    kset24=[7*round(m/K)+1:m];
    [alphaTri24,wTri24,tAvg24]=local_SVM(w,alpha24,m,lambda,A(:,kset24),y0(kset24,1),H);
    ...
    w=w+(wTri11+wTri12+wTri13+wTri14+wTri21+wTri22+wTri23+wTri24)/K;
