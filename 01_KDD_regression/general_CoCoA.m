function [dualGap,tOP,T]= general_CoCoA(dataset, MaxItrOut, MaxItrIn, weight_wor_sub, weight_Cen_Sub,H)
...
for T=1:MaxItrOut
    for L=1:MaxItrIn
        ...
        %% local worker #1
        kset1=[1:round(m/K)];
        [alphaTri1,wTri1,tAvg1]=localSDCA(w_sub1,alpha1,m,lambda,A(:,kset1),y0(kset1,1),H);
        alpha1 = alpha1 + alphaTri1;        
        %% local worker #2
        kset2=[round(m/K)+1:2*round(m/K)];
        [alphaTri2,wTri2,tAvg2]=localSDCA(w_sub1,alpha2,m,lambda,A(:,kset2),y0(kset2,1),H);
        alpha2 = alpha2 + alphaTri2;           
        %% local worker #3
        kset3=[2*round(m/K)+1:3*round(m/K)];
        [alphaTri3,wTri3,tAvg3]=localSDCA(w_sub1,alpha3,m,lambda,A(:,kset3),y0(kset3,1),H);
        alpha3 = alpha3 + alphaTri3;            
        %% local worker #4
        kset4=[3*round(m/K)+1:4*round(m/K)];
        [alphaTri4,wTri4,tAvg4]=localSDCA(w_sub1,alpha4,m,lambda,A(:,kset4),y0(kset4,1),H);
        alpha4 = alpha4 + alphaTri4;          
        %% local worker #5
        kset5=[4*round(m/K)+1:5*round(m/K)];
        [alphaTri5,wTri5,tAvg5]=localSDCA(w_sub1,alpha5,m,lambda,A(:,kset5),y0(kset5,1),H);
        alpha5 = alpha5 + alphaTri5;  
        
        
        %% local worker #6
        kset6=[5*round(m/K)+1:6*round(m/K)];
        [alphaTri6,wTri6,tAvg6]=localSDCA(w_sub2,alpha6,m,lambda,A(:,kset6),y0(kset6,1),H);
        alpha6 = alpha6 + alphaTri6;        
        %% local worker #7
        kset7=[6*round(m/K)+1:7*round(m/K)];
        [alphaTri7,wTri7,tAvg7]=localSDCA(w_sub2,alpha7,m,lambda,A(:,kset7),y0(kset7,1),H);
        alpha7 = alpha7 + alphaTri7;           
        %% local worker #8
        kset8=[7*round(m/K)+1:8*round(m/K)];
        [alphaTri8,wTri8,tAvg8]=localSDCA(w_sub2,alpha8,m,lambda,A(:,kset8),y0(kset8,1),H);
        alpha8 = alpha8 + alphaTri8;            
        %% local worker #9
        kset9=[8*round(m/K)+1:9*round(m/K)];
        [alphaTri9,wTri9,tAvg9]=localSDCA(w_sub2,alpha9,m,lambda,A(:,kset9),y0(kset9,1),H);
        alpha9 = alpha9 + alphaTri9;          
        %% local worker #10
        kset10=[9*round(m/K)+1:10*round(m/K)];
        [alphaTri10,wTri10,tAvg10]=localSDCA(w_sub2,alpha10,m,lambda,A(:,kset10),y0(kset10,1),H);
        ...
        w_sub1_tri=(wTri1+wTri2+wTri3+wTri4+wTri5)/5;
        w_sub1=w_sub1+w_sub1_tri;
        
        % subcenter 2 updating        
        w_sub2_tri=(wTri6+wTri7+wTri8+wTri9+wTri10)/5;
    ...
    alpha = [alpha1;alpha2;alpha3;alpha4;alpha5;alpha6;alpha7;alpha8;alpha9;alpha10]; % for duality gap check purpose
    dualGap=[dualGap;P(w)-D(alpha)];

    tCOMMtot=tCOMMtot+toc(tCOMM_SubC2C); 
    ts= tCOMMtot+tCOMM_L2SubC_Tot+tOPtot;
    tOP=[tOP;ts];
